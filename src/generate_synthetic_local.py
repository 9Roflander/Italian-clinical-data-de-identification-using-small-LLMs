...
def generate_dataset(
    ...
    # Use OpenAI client configured for local vLLM or Ollama endpoint
    client = OpenAI(
        base_url=base_url,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate synthetic Italian clinical de-identification dataset via Local LLM (Mistral NeMo or others)."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Il nome del modello locale da utilizzare (Ollama, vLLM, ecc.), default: mistral-nemo",
    )
    parser.add_argument(
        "--base-url",
        type=str,
        default="http://localhost:11434/v1",
        help="URL di base per le API OpenAI-compatibili (default Ollama: http://localhost:11434/v1, vLLM: http://localhost:8000/v1)",
    )
